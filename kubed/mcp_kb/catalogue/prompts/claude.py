"""Claude Code commands: declared names, `$ARGUMENTS`, and `$1`-`$9`.
...
TOKEN = re.compile(
    r"\$ARGUMENTS\[(\d+)\]"  # 1: the N-th, indexed
    r"|\$ARGUMENTS\b"  # everything typed after the command
    rf"|{_NTH}"  # 2: the N-th, positional
...
def substitute(body: str, values: dict[str, str]) -> str:
    ...
    ``FilePrompt.render`` keeps), which is what makes `$1` resolvable: it is
    ...
    def nth(index: int) -> str:
        """The N-th declared name's value, or the N-th word of the free text."""
        if names:
            return values.get(names[index - 1], "") if 1 <= index <= len(names) else ""
        words = free.split()
        return words[index - 1] if 1 <= index <= len(words) else ""

    def fill(match: re.Match) -> str:
        indexed, positional, name = match.groups()
        if indexed is not None:
            return nth(int(indexed))
        if positional is not None:
            return nth(int(positional))
        if name is not None:
