...
### Changed
- `pyproject.toml` names `mcp_school.sources` explicitly rather than leaning on setuptools_scm's file finder to sweep it into the wheel, and the suite now builds a wheel from a checkout with no `.git` and imports from it.
